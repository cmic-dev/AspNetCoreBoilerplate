...
public class AuthService
{
    ...
    private const string GENERIC_AUTH_ERROR = "Invalid username or password";

    public AuthService(JwtProvider jwtProvider, PasswordProvider passwordProvider,
        IAppDbContext dbContext, IUserContext userContext)
    {
    ...
    public async Task<AuthResponseDto> LoginAsync(LoginRequestDto request, CancellationToken ctn = default)
    {
        ...
        if (user.RequiresPasswordChange)
            throw new PasswordChangeRequiredException(GENERIC_AUTH_ERROR);
        ...
        return new AuthResponseDto
        {
            ...
            UserInfo = new UserInfoDto(user.Id,
                user.UserName,
                user.DisplayName,
                user.Email,
                user.Role.Name,
